...
Six Six::add(const Six& other) const{
    ...
    unsigned char step = 0; // Перенос в следующий разряд
    
    for (size_t i = 0; i < maxSize || step; ++i) {
        unsigned char sum = step;
...
Six Six::subtract(const Six& other) const{
    if (lessThan(other)) throw std::invalid_argument("Cannot subtract larger number");
    ...
    for (size_t i = 0; i < digits.size(); ++i) {
        int current = digits[i] - step;
        // Цифра вычитаемого (0 если разряд отсутствует)
